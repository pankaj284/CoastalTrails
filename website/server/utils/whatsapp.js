...
// Policy: Only welcome message and booking confirmed message are allowed via WhatsApp.
// All other messages (payment receipts, payment pending/failed, reminders, etc.) are strictly disabled.
export async function sendPaymentWhatsApp(_booking, _homestay) {
  return { sent: false, provider: null, suppressed: true, reason: 'WhatsApp messages restricted to welcome and booking confirmation only' };
}

export async function sendPaymentFailedWhatsApp(_booking, _homestay) {
  return { sent: false, provider: null, suppressed: true, reason: 'WhatsApp messages restricted to welcome and booking confirmation only' };
}
...
export async function sendSignupWhatsApp(user) {
  ...
  const templateName = process.env.WHATSAPP_SIGNUP_TEMPLATE || 'ct_welcome';
