...
router.get('/stats', async (req, res) => {
  try {
    const [stays, bookings, hosts, holds, upcoming, statusRows, arrivals, departures] = await Promise.all([
      ...
      get(`SELECT COUNT(*) AS c FROM bookings WHERE check_in >= date('now') AND status NOT IN ('declined', 'cancelled')`),
    ...
    if (status === 'available') {
      ...
      await run(
        `INSERT INTO room_status (homestay_id, room_number, date, status, reason) VALUES (?, ?, ?, ?, ?)
         ON CONFLICT(homestay_id, room_number, date) DO UPDATE SET status = excluded.status, reason = excluded.reason, updated_at = CURRENT_TIMESTAMP`,
  ...
  try {
    ...
    await run(
      `INSERT INTO room_state (homestay_id, room_number, name, bed_type, capacity, housekeeping, photo)
       VALUES (?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT(homestay_id, room_number) DO UPDATE SET
         name = COALESCE(excluded.name, room_state.name),
         bed_type = COALESCE(excluded.bed_type, room_state.bed_type),
         capacity = COALESCE(excluded.capacity, room_state.capacity),
         housekeeping = COALESCE(excluded.housekeeping, room_state.housekeeping),
         photo = COALESCE(excluded.photo, room_state.photo),
    ...
    await run(
      `INSERT INTO stay_settings (homestay_id, min_stay, price_override) VALUES (?, ?, ?)
       ON CONFLICT(homestay_id) DO UPDATE SET
         min_stay = COALESCE(excluded.min_stay, stay_settings.min_stay),
         price_override = COALESCE(excluded.price_override, stay_settings.price_override),
         updated_at = CURRENT_TIMESTAMP`,
