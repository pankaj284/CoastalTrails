...
import { guestWhatsAppLink, hostWhatsAppLink, sendPaymentWhatsApp } from '../utils/whatsapp.js';
...
router.post('/:bookingId/confirm', requireAuth, async (req, res) => {
  try {
    ...
    const paymentWhatsapp = await sendPaymentWhatsApp(updated);
    console.log(
      paymentWhatsapp.sent
        ? `WhatsApp payment receipt sent to ${updated.user_phone} (${paymentWhatsapp.provider})`
        : `WhatsApp payment send failed for ${updated.user_phone}: ${paymentWhatsapp.reason}`
    );
